	...
	public void cadastraVeiculos() {
		...
		do {
			try {
				...
				if (String.valueOf(ano).length() < 4) {
					JOptionPane.showMessageDialog(null, "Por favor, insira um ano com no mínimo 4 dígitos.");
					ano = -1;
				}
		...
		do {
			try {
				...
				if (qtdPessoas < 1) {
					JOptionPane.showMessageDialog(null, "A quantidade de pessoas deve ser de no minimo 1.");
				}
			} catch (NumberFormatException e) {
				JOptionPane.showMessageDialog(null, "Por favor, insira um número válido.");
				qtdPessoas = -1; // Define um valor inválido para repetir o loop
			}
		} while (qtdPessoas == -1);
